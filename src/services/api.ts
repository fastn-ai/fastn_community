        ...
        try {
          ...
          if (response && response.result) {
            ...
            return replies.map((reply: any) => ({
              ...
              like_count: typeof reply.like_count === 'number' ? reply.like_count : (reply.like_count ? parseInt(String(reply.like_count), 10) : 0),
              dislike_count: typeof reply.dislike_count === 'number' ? reply.dislike_count : (reply.dislike_count ? parseInt(String(reply.dislike_count), 10) : 0),
              reply_count: typeof reply.reply_count === 'number' ? reply.reply_count : (reply.reply_count ? parseInt(String(reply.reply_count), 10) : 0),
...
export async function removeLikeFromTopicApi(payload: { data: { user_id: string; topic_id: number } }, authToken: string, apiKey?: string) {
  ...
  const res = await fetch(REMOVE_LIKE_FROM_TOPIC_API_URL, {
    method: "POST",
    headers,
    body: JSON.stringify({ input: payload }),
  });
  ...
  if (!res.ok) {
    ...
    const error = new Error(`removeLikeFromTopic failed: ${res.status} ${res.statusText} - ${errorMessage}`);
    // Attach the original response for additional context
    (error as any).response = { status: res.status, text };
    (error as any).errorJson = errorJson; // Attach parsed JSON for easier access
    throw error;
...
/* export async function removeLikeFromReplyApi(payload: { data: { user_id: string; reply_id: number } }, authToken: string, apiKey?: string) {
  console.log("removeLikeFromReplyApi called with:", { payload, authToken: authToken ? "present" : "missing" });
  ...
  const res = await fetch(REMOVE_LIKE_FROM_REPLY_API_URL, {
    method: "POST",
    headers,
    body: JSON.stringify({ input: payload }),
  });
  ...
  if (!res.ok) {
    ...
    const error = new Error(`removeLikeFromReply failed: ${res.status} ${res.statusText} - ${errorMessage}`);
    // Attach the original response for additional context
    (error as any).response = { status: res.status, text };
    (error as any).errorJson = errorJson; // Attach parsed JSON for easier access
    throw error;
...
export const useApi = () => {
  return {
    ...
    removeLikeFromTopicApi,
    // removeLikeFromReplyApi,
  };
};